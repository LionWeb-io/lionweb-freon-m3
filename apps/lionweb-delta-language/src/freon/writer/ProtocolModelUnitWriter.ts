...
import { PropertyDef, PrimitiveType, ObjectType, type Type, Types, MessageGroup } from "../language/index.js";
...
/**
 * Class ProtocolModelUnitWriter provides methods to return a string representation of an instance of
 * elements of language Protocol.
 * It is, amongst others, used to create error messages in the validator.
 */
export class ProtocolModelUnitWriter implements FreWriter {
    ...
    public writeNameOnly(node: FreNode | undefined): string {
        ...
        if (node instanceof Types) {
            return node.name;
        } else if (node instanceof MessageGroup) {
            return node.name;
        } else if (node instanceof PropertyDef) {
            return node.name;
    ...
    private unparse(node: FreNode, short: boolean) {
        ...
        switch (node.freLanguageConcept()) {
            case "PropertyDef":
                this.unparsePropertyDef(node as PropertyDef, short);
                break;
            case "PrimitiveType":
                this.unparsePrimitiveType(node as PrimitiveType, short);
                break;
            case "ObjectType":
                this.unparseObjectType(node as ObjectType, short);
                break;
            case "Types":
                this.unparseTypes(node as Types, short);
                break;
            case "MessageGroup":
                this.unparseMessageGroup(node as MessageGroup, short);
                break;
            case "Type":
                this.unparseType(node as Type, short);
                break;
            default:
    ...
    /**
     * Unparsing of 'Types' according to projection 'default'.
     */
    private unparseTypes(node: Types, short: boolean) {
        const blockIndent = this.output[this.currentLine].length;
        // do the first line
        this.output[this.currentLine] += `Type Definitions `;
        ...
        if (!short) {
            ...
            ("");
            this.newlineAndIndentation(blockIndent + 0);
            this.output[this.currentLine] += `Primitives: `;
            ...
            ("");
            this.newlineAndIndentation(blockIndent + 0);
            this.output[this.currentLine] += `Objects: `;
            this.newlineAndIndentation(blockIndent + 4);
            this._unparseList(
                node.objectTypes,
                "",
    ...
    private unparsePrimitiveType(node: PrimitiveType, short: boolean) {
        this.output[this.currentLine] += `primitive `;
        this.output[this.currentLine] += `\`${node.name}\` ` /* 3 */;
        this.output[this.currentLine] += `: `;
        this.output[this.currentLine] += notNullOrUndefined(node.primitiveType) ? `"${node.primitiveType}" ` : `""` /* 5 */;
    }

    /**
     * Unparsing of 'ObjectType' according to projection 'default'.
     */
    private unparseObjectType(node: ObjectType, short: boolean) {
        const blockIndent = this.output[this.currentLine].length;
        // do the first line
        this.output[this.currentLine] += `object `;
        this.output[this.currentLine] += `\`${node.name}\` ` /* 3 */;
    ...
    /**
     * Unparsing of 'PropertyDef' according to projection 'default'.
     */
    private unparsePropertyDef(node: PropertyDef, short: boolean) {
        this.output[this.currentLine] += `\`${node.name}\` ` /* 3 */;
        this.output[this.currentLine] += `: `;
    ...
    /**
     * Unparsing of 'MessageGroup' according to projection 'default'.
     */
    private unparseMessageGroup(node: MessageGroup, short: boolean) {
        const blockIndent = this.output[this.currentLine].length;
        // do the first line
        this.output[this.currentLine] += `MessageGroup `;
        this.output[this.currentLine] += `\`${node.name}\` ` /* 3 */;
        ...
        if (!short) {
            ...
            ("");
            this.newlineAndIndentation(blockIndent + 0);
            this.output[this.currentLine] += `TaggedUnion property name `;
            this.output[this.currentLine] += notNullOrUndefined(node.taggedUnionProperty) ? `"${node.taggedUnionProperty}" ` : `""` /* 5 */;
            this.newlineAndIndentation(blockIndent + 0);
            this.output[this.currentLine] += `Shared properties: `;
            this.newlineAndIndentation(blockIndent + 4);
            this._unparseList(
                node.sharedProperties,
                "",
                SeparatorType.Separator,
                true,
                this.output[this.currentLine].length,
            ...
            ("");
            this.newlineAndIndentation(blockIndent + 0);
            this.output[this.currentLine] += `Messages `;
            this.newlineAndIndentation(blockIndent + 0);
            this._unparseList(
                node.messages,
                "",
     ...
     */
    private unparseType(node: FreNode, short: boolean) {
