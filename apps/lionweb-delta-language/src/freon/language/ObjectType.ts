...
import { type Type, PropertyDef } from "./internal.js";

// TEMPLATE: ConceptTemplate.generateConceptPrivate
import { makeObservable, action } from "mobx";

/**
 * Class ObjectType is the implementation of the concept with the same name in the language definition file.
 * It uses mobx decorators to enable parts of the language environment, e.g. the editor, to react
 * to the changes in the state of its properties.
 */
export class ObjectType extends MobxModelElementImpl implements FreNamedNode, Type {
    /**
     * A convenience method that creates an instance of this class
     * based on the properties defined in 'data'.
     * @param data partial object
     */
    static create(data: Partial<ObjectType>): ObjectType {
        const result = new ObjectType(data.$id);
        if (notNullOrUndefined(data.name)) {
            result.name = data.name;
        }
        if (notNullOrUndefined(data.properties)) {
    ...
    readonly $typename: string = "ObjectType"; // holds the metatype in the form of a string
    ...
    properties!: PropertyDef[]; // implementation of part 'properties'
    ...
    copy(): ObjectType {
        ...
        if (notNullOrUndefined(this.properties)) {
     ...
     * @param toBeMatched
     */
    public match(toBeMatched: Partial<ObjectType>): boolean {
        let result: boolean = true;
        if (result && toBeMatched.name !== null && toBeMatched.name !== undefined && toBeMatched.name.length > 0) {
            result = result && this.name === toBeMatched.name;
        }
        if (result && notNullOrUndefined(toBeMatched.properties)) {
