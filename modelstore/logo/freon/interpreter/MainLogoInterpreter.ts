...
import { LogoInterpreterInit } from "./gen/LogoInterpreterInit.js";
 ...
 */
export class MainLogoInterpreter implements FreInterpreter {
    private static main: IMainInterpreter | null = null;

    constructor() {
        MainLogoInterpreter.getMain();
    }

    private static getMain(): IMainInterpreter {
        return (this.main ??= MainInterpreter.instance(LogoInterpreterInit, getConceptFunction, getPropertyFunction));
    }

    setTracing(value: boolean) {
        MainLogoInterpreter.getMain().setTracing(value);
    }

    getTrace(): InterpreterTracer {
        return MainLogoInterpreter.getMain().getTrace();
    }
    ...
    evaluateWithContext(node: Object, ctx: InterpreterContext): RtObject {
        MainLogoInterpreter.getMain().reset();
        try {
            return MainLogoInterpreter.getMain().evaluate(node, ctx);
        } catch (e: any) {
