...
import { Procedure, type INamed } from "./internal.js";

import { makeObservable, action } from "mobx";

/**
 * Class Library is the implementation of the model unit with the same name in the language definition file.
 * It uses mobx decorators to enable parts of the language environment, e.g. the editor, to react
 * to any changes in the state of its properties.
 */
export class Library extends MobxModelElementImpl implements FreModelUnit, INamed {
    /**
     * A convenience method that creates an instance of this class
     * based on the properties defined in 'data'.
     * @param data partial object
     */
    static create(data: Partial<Library>): Library {
        const result = new Library(data.$id);
        if (notNullOrUndefined(data.name)) {
            result.name = data.name;
        }
        if (notNullOrUndefined(data.procedures)) {
            result.procedures = data.procedures;
        } else {
            result.procedures = Procedure.create({});
        }
    ...
    readonly $typename: string = "Library"; // holds the metatype in the form of a string
    ...
    procedures!: Procedure; // implementation of part 'procedures'
    ...
    constructor(id?: string) {
        ...
        observablepart(this, "procedures");
    ...
    copy(): Library {
        ...
        if (notNullOrUndefined(this.procedures)) {
            result.procedures = this.procedures.copy();
        }
        return result;
    }
    /**
     ...
     * @param toBeMatched
     */
    public match(toBeMatched: Partial<Library>): boolean {
        let result: boolean = true;
        if (result && toBeMatched.name !== null && toBeMatched.name !== undefined && toBeMatched.name.length > 0) {
            result = result && this.name === toBeMatched.name;
        }
        if (result && notNullOrUndefined(toBeMatched.procedures)) {
            result = result && this.procedures.match(toBeMatched.procedures);
        }
        return result;
    }
}
